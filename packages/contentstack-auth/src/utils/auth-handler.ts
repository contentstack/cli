...
class AuthHandler {
  ...
  constructor() {
    ...
    this.OAuthScope =
      process.env.OAUTH_APP_SCOPE_PERMISSIONS || 'user:read cm.stacks.management:read organization:read';
  ...
  async compareOAuthExpiry(force: boolean = false) {
    ...
    if (oauthDateTime && authorisationType === this.authorisationTypeOAUTHValue) {
      ...
      //   console.log(oauthDate);
      //   console.log(oauthValidUpto);
      //   console.log(now);
      if (oauthValidUpto > now && !force) {
