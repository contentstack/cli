...
async function getLocalizedEntry(stack, entry, contentType, locale, sourceEnv) {
  return new Promise((resolve, _reject) => {
    ...
    stack.contentType(contentType).entry(entry.uid).fetch(queryParams)
    .then(entryResponse => {
      if (entryResponse) {
...
async function getEntries(stack, schema, contentType, languages, masterLocale, bulkPublish, environments, sourceEnv, skip = 0) {
  return new Promise((resolve, reject) => {
    skipCount = skip
    let queryParams = {
            ...
            try {
              ...
              if (checkNonLocalizedFieldChanges(schema, entry, localizedEntry)) {
                if (bulkPublish) {
                  ...
                  if (bulkPublishSet.length === 10) {    
                    await queue.Enqueue({
                      entries: bulkPublishSet, locale: locale.code, Type: 'entry', environments: environments, stack: stack
                    })
                    bulkPublishSet = []
                  }
                } else {
                  await queue.Enqueue({
                    content_type: contentType, publish_details: entry.publish_details || [], environments: environments, entryUid: entry.uid, locale: locale.code, Type: 'entry', stack: stack
                  })
                }
              } else {
