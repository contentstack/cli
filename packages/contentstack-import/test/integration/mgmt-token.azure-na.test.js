...
const { NA_AZ_USERNAME: USERNAME, NA_AZ_PASSWORD: PASSWORD } = getEnvData();

async function exec() {
  const __stackDetails = getStackDetails();
  const stack = Object.keys(__stackDetails).find(el => el.includes('NA_AZ'));
  ...
  describe('NA-AZURE-Contentstack-import plugin test using mgmt token [--alias=common_mgmt_token_na_az]', () => {
    customTest
      .timeout(DEFAULT_TIMEOUT || 600000) // NOTE: setting default as 10 minutes
      .stdout({ print: PRINT_LOGS || false })
      .command(RegionSetCommand, ['AZURE-NA'])
    ...
    describe('NA-AZURE-Import complex stack using management token', () => {
      test
        .timeout(DEFAULT_TIMEOUT || 600000) // NOTE: setting default as 10 minutes
        .stdout({ print: PRINT_LOGS || false })
        ...
        .command(['cm:stacks:import', '-a', 'common_mgmt_token_na_az'])
        .it('should work without any errors', (_, done) => {
          done();
        });
    });

    describe('NA-AZURE-Check if all content-types are imported correctly', () => {
      test
        .timeout(DEFAULT_TIMEOUT || 600000) // NOTE: setting default as 10 minutes
        .stdout({ print: PRINT_LOGS || false })
    ...
    describe('NA-AZURE-Check if all entries are imported correctly', () => {
      test
        .timeout(DEFAULT_TIMEOUT || 600000) // NOTE: setting default as 10 minutes
        .stdout({ print: PRINT_LOGS || false })
