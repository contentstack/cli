...
module.exports = region => {
  ...
  for (const stack of Object.keys(stackDetails)) {
    ...
    describe('Contentstack-import plugin test [--module=workflows]', () => {
      ...
      customTest
        .stdout({ print: PRINT_LOGS || false })
        .command(ExportCommand, ['--alias', stackDetails[stack].EXPORT_ALIAS_NAME, '--data-dir', basePath, '--module', 'workflows'])
        .it('should work without any errors', (_, done) => {
          done();
        });
      ...
      describe('Import assets using cm:stacks:import command', () => {
        test
          .stdout({ print: PRINT_LOGS || false })
          .command(['cm:stacks:import', '--alias', stackDetails[stack].ALIAS_NAME, '--data-dir', importBasePath, '--module', 'workflows'])
