...
const { getStackDetailsByRegion, cleanUp, getEnvData, getWorkflowsCount } = require('../utils/helper');
...
module.exports = region => {
  ...
  for (const stack of Object.keys(stackDetails)) {
    ...
    describe('Contentstack-import plugin test with auth token [--module=workflows]', () => {
      customTest
      ...
      customTest
        .stdout({ print: PRINT_LOGS || false })
        .command(ExportCommand, ['--stack-api-key', stackDetails[stack].EXPORT_STACK_API_KEY, '--data-dir', basePath, '--module', 'workflows'])
        .it('should work without any errors', (_, done) => {
          done();
        });
      ...
      describe('Import assets using cm:stacks:import command', () => {
        test
          .stdout({ print: PRINT_LOGS || false })
          .command(['cm:stacks:import', '--stack-api-key', stackDetails[stack].STACK_API_KEY, '--data-dir', importBasePath, '--module', 'workflows'])
