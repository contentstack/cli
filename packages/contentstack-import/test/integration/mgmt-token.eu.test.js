...
const { EU_USERNAME: USERNAME, EU_PASSWORD: PASSWORD } = getEnvData();

async function exec() {
  const __stackDetails = getStackDetails();
  const stack = Object.keys(__stackDetails).find(el => el.includes('EU'));
  ...
  describe('EU-Contentstack-import plugin test using mgmt token [--alias=common_mgmt_token_eu]', () => {
    customTest
      .timeout(DEFAULT_TIMEOUT || 600000) // NOTE: setting default as 10 minutes
      .stdout({ print: PRINT_LOGS || false })
      .command(RegionSetCommand, ['EU'])
    ...
    describe('EU-Import complex stack using management token', () => {
      test
        .timeout(DEFAULT_TIMEOUT || 600000) // NOTE: setting default as 10 minutes
        .stdout({ print: PRINT_LOGS || false })
        ...
        .command(['cm:stacks:import', '-a', 'common_mgmt_token_eu'])
        .it('should work without any errors', (_, done) => {
          done();
        });
    });

    describe('EU-Check if all content-types are imported correctly', () => {
      test
        .timeout(DEFAULT_TIMEOUT || 600000) // NOTE: setting default as 10 minutes
        .stdout({ print: PRINT_LOGS || false })
    ...
    describe('EU-Check if all entries are imported correctly', () => {
      test
        .timeout(DEFAULT_TIMEOUT || 600000) // NOTE: setting default as 10 minutes
        .stdout({ print: PRINT_LOGS || false })
