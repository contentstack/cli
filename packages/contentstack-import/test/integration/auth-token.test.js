...
const { getStackDetailsByRegion, getContentTypesCount, cleanUp, getEnvData, getEntriesCount } = require('./utils/helper');
...
module.exports = (region) => {
  ...
  for (let stack of Object.keys(stackDetails)) {
    ...
    describe('Contentstack-import test using auth token [--alias=ALIAS]', () => {
      customTest
      ...
      customTest
        .stdout({ print: PRINT_LOGS || false })
        .command(ExportCommand, ['--stack-api-key', stackDetails[stack].EXPORT_STACK_API_KEY, '--data-dir', basePath])
        .it('should work without any errors', (_, done) => {
          done();
        });
      ...
      describe('Import stack using cm:stacks:import command', () => {
        test
          .stdout({ print: PRINT_LOGS || false })
          .command(['cm:stacks:import', '--stack-api-key', stackDetails[stack].STACK_API_KEY, '--data-dir', importBasePath])
          .it('should work without any errors', (_, done) => {
            done();
          });
      });
