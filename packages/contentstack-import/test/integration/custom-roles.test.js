...
const { getStackDetailsByRegion, cleanUp, deleteStack, getEnvData, getCustomRolesCount } = require('./utils/helper');
...
module.exports = region => {
  ...
  for (const stack of Object.keys(stackDetails)) {
    ...
    describe('Contentstack-import plugin test [--module=custom-roles]', () => {
      customTest
      ...
      customTest
        .stdout({ print: PRINT_LOGS || false })
        .command(ExportCommand, ['--alias', stackDetails[stack].EXPORT_ALIAS_NAME, '--data-dir', basePath, '--module', 'custom-roles'])
        .it('should work without any errors', (_, done) => {
          done();
        });
      ...
      describe('Import assets using cm:stacks:import command', () => {
        test
          .stdout({ print: PRINT_LOGS || false })
          .command(['cm:stacks:import', '--alias', stackDetails[stack].ALIAS_NAME, '--data-dir', importBasePath, '--module', 'custom-roles'])
