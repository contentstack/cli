  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (subDir) {
      ...
      if (importConfig.createBackupDir) {
        cliux.print(
          `Warning!!! Provided backup directory path is a sub directory of the content directory, Cannot copy to a sub directory. So new backup directory created - ${backupDirPath}`,
          {
            color: 'yellow',
          },
        );
