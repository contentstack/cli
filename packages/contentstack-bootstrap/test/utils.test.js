...
describe('Utils', () => {
    describe('#setupEnvironments', () => {
        it('Create env file for a stack', async () => {
            ...
            const region = {
                name: 'NA',
                cda: 'https://stag-app.contentstack.com',
            ...
            expect(devEnvFile).equal('REACT_APP_APIKEY=mock-api-key,REACT_APP_DELIVERY_TOKEN=mock-delivery-token,REACT_APP_ENVIRONMENT=development,REACT_APP_REGION=NA')
            let prodEnvFile = await getFileContent(path.join(clonedDirectory, '.env.production.local'));
            prodEnvFile = prodEnvFile.replace(/\n/g, ',')
            expect(prodEnvFile).equal('REACT_APP_APIKEY=mock-api-key,REACT_APP_DELIVERY_TOKEN=mock-delivery-token,REACT_APP_ENVIRONMENT=production,REACT_APP_REGION=NA')
        })
