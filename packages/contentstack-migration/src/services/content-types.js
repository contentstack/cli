...
const { parse } = JSON,
  ...
  // Utils
  { map: _map, safePromise, successHandler, errorHandler, constants } = require('../utils'),
  // Map methods
  { get, getMapInstance, getDataWithAction } = _map,
  mapInstance = getMapInstance(),
  { ContentType, MANAGEMENT_SDK } = constants;
...
class ContentTypeService {
  ...
  async deleteContentType(callsite) {
    const { id } = this,
      method = 'DELETE',
      [err, result] = await safePromise(this.stackSDKInstance.contentType(id).delete());
  ...
  applyActionsOnFields(callsite, data, cb) {
    const { schema } = data,
      { moveFieldActions, mergeEditSchema } = this;
  ...
  mergeEditSchema(schema = []) {
    ...
    const indicesToRemoveFromNewSchema = [],
      indicesToRemoveFromOldSchema = [];

    let isEditFieldValid = false,
      isEditFieldPresent = false,
      isDeleteFieldValid = false,
      isDeleteFieldPresent = false,
  ...
  afterField(schema, actionObj) {
    ...
    let i = 0, indexToMove = 0, tempObj, found = 0;
    // eslint-disable-next-line
    while (true) {
  ...
  beforeField(schema, actionObj) {
    ...
    let i = 0, indexToMove = 0, tempObj = 0, found = 0;
    // eslint-disable-next-line
    while (true) {
      if (schema[i].uid === against) {
        indexToMove = i;
  ...
  getValidated(schema, actionObj) {
    ...
    const { fieldToMove, against } = actionObj, uids = [];
    // eslint-disable-next-line
    while (true) {
