...
module.exports = class ExportCustomRoles {
  ...
  async start() {
    ...
    try {
      ...
      const customRoles = roles.items.filter(
        (role) => !self.EXISTING_ROLES[role.name]
      );

      if (!customRoles.length) {
        addlogs(
          self.config,
          "No custom roles were found in the Stack",
          "success"
        );
        return;
      }
      ...
      customRoles.forEach((role) => {
        addlogs(
          self.config,
          role.name + " role was exported successfully",
          "success"
        );
        self.customRoles[role.uid] = role;
      });
      helper.writeFileSync(
        path.join(rolesFolderPath, self.rolesConfig.fileName),
        self.customRoles
      );
      addlogs(
        self.config,
        chalk.green("All the custom roles have been exported successfully"),
        "success"
      );
    } catch (error) {
      ...
      addlogs(
        self.config,
        "Error occurred in exporting roles. " + error && error.message,
        "error"
      );
