    ...
    return new Promise(function (resolve, reject) {
      client
        ...
        .then((extension) => {
          if (extension.items.length !== 0) {
            ...
            helper.writeFileSync(
              path.join(extensionsFolderPath, self.extensionConfig.fileName),
              self.extensions
            );
            addlogs(
              self.config,
              chalk.green("All the extensions have been exported successfully"),
              "success"
            );
            return resolve();
          }
        ...
        .catch((error) => {
          addlogs(
            self.config,
            `Failed to export extensions ${formatError(error)}`,
            "error"
          );
          reject();
        });
