...
module.exports = class ExportLabels {
  ...
  start() {
    ...
    let labelsFolderPath = path.resolve(
      config.data,
      this.config.branchName || "",
      self.labelConfig.dirName
    );
        ...
        .then((response) => {
          if (response.items.length !== 0) {
            response.items.forEach(function (label) {
              addlogs(
                self.config,
                label.name + " labels was exported successfully",
                "success"
              );
            ...
            addlogs(
              self.config,
              chalk.green("All the labels have been exported successfully"),
              "success"
            );
          } else {
          ...
          helper.writeFileSync(
            path.join(labelsFolderPath, self.labelConfig.fileName),
            self.labels
          );
          resolve();
        })
