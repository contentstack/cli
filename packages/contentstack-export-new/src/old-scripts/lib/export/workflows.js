...
module.exports = class ExportWorkFlows {
  ...
  start() {
    ...
    let workflowsFolderPath = path.resolve(
      this.config.data,
        ...
        .then(async (response) => {
          try {
            if (response.items.length) {
              ...
              addlogs(
                self.config,
                chalk.green("All the workflow have been exported successfully"),
                "success"
              );
            }
            if (!response.items.length) {
              addlogs(
                self.config,
                "No workflow were found in the Stack",
                "success"
              );
            }
            helper.writeFileSync(
              path.join(workflowsFolderPath, self.workFlowConfig.fileName),
              self.workflows
            );
            resolve();
          } catch (error) {
  ...
  async getWorkflowRoles(self, workflow) {
    try {
      ...
      addlogs(
        self.config,
        "Error fetching roles in export workflows task.",
        "error"
      );
      throw new Error({
        message: "Error fetching roles in export workflows task.",
      });
    }
  }
  ...
  async getWorkflowsData(self, workflows) {
    try {
      for (const workflow of workflows) {
        addlogs(
          self.config,
          workflow.name + " workflow was exported successfully",
          "success"
        );
