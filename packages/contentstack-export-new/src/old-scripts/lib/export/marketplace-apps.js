...
} = require("../util/marketplace-app-helper");
...
module.exports = class ExportMarketplaceApps {
  ...
  async start() {
    this.developerHubBaseUrl =
      this.config.developerHubBaseUrl || (await getDeveloperHubUrl());
      ...
      getInstalledExtensions(self.config)
        .then(async (items) => {
          ...
          for (const app of uniqApps) {
            log(
              self.config,
              `Exporting ${app.title} app and it's config.`,
              "success"
            );
            ...
            await httpClient
              .get(
                `${self.developerHubBaseUrl}/installations/${app.app_installation_uid}/installationData`
              )
              .then(async ({ data: result }) => {
                const { data, error } = result;
                const developerHubApp = _.find(developerHubApps, {
                  uid: app.app_uid,
                });

                if (
                  (_.has(data, "configuration") ||
                    _.has(data, "server_configuration")) &&
                  !self.nodeCrypto
                ) {
                ...
                _.forEach(listOfIndexToBeUpdated, (index, i) => {
                  if (developerHubApp) {
                    installedApps[index]["visibility"] =
                      developerHubApp.visibility;
                  ...
                  if (
                    ...
                    if (!_.isEmpty(configuration)) {
                      installedApps[index]["configuration"] =
                        self.nodeCrypto.encrypt(configuration);
                    }
                    if (!_.isEmpty(server_configuration)) {
                      installedApps[index]["server_configuration"] =
                        self.nodeCrypto.encrypt(server_configuration);
                    }

                    if (i === 0) {
                      log(
                        self.config,
                        `Exported ${app.title} app and it's config.`,
                        "success"
                      );
                    }
                  } else if (error) {
                    ...
                    if (i === 0) {
                      log(
                        self.config,
                        `Error on exporting ${app.title} app and it's config.`,
                        "error"
                      );
                    }
                  }
              ...
              .catch((err) => {
                log(
                  self.config,
                  `Failed to export ${app.title} app config ${formatError(
                    error
                  )}`,
                  "error"
                );
          ...
          if (!_.isEmpty(installedApps)) {
            await writeFileSync(
              path.join(
                self.marketplaceAppPath,
                self.marketplaceAppConfig.fileName
              ),
              installedApps
            );
        ...
        .catch((error) => {
          log(
            self.config,
            `Failed to export marketplace-apps ${formatError(error)}`,
            "error"
          );
          reject(error);
        });
