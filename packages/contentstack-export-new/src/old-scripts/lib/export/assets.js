...
module.exports = class ExportAssets {
  ...
  constructor(config) {
    ...
    this.vLimit =
      this.assetConfig.downloadLimit || config.fetchConcurrency || 3;
  }

  start() {
    const self = this;
    this.assetsFolderPath = path.resolve(
      this.config.data,
          ...
          return Promise.map(
                ...
                .then(function (assetsJSON) {
                  return Promise.map(
                        ...
                        return self
                          ...
                          .catch(function (error) {
                            addlogs(
                              self.config,
                              chalk.red(
                                "The following asset failed to download\n" +
                    ...
                    .then(function () {
                      addlogs(
                        self.config,
                        "Batch no " + (batch + 1) + " of assets is complete",
                        "success"
                      );
                    ...
                    .catch(function (error) {
                      console.log(
                        "Error fetch/download the asset",
                        error && error.message
                      );
                      addlogs(
                        self.config,
                        "Asset batch " + (batch + 1) + " failed to download",
                        "error"
                      );
                      addlogs(self.config, error, "error");
                    });
            ...
            .then(function () {
              ...
              return self
                ...
                .then(function () {
                  addlogs(
                    self.config,
                    chalk.green("Asset export completed successfully"),
                    "success"
                  );
                  return resolve();
                })
            ...
            .catch(function (error) {
              ...
              addlogs(
                self.config,
                chalk.red(
                  "Asset export failed due to the following errors " +
      ...
      return self
        ...
        .then(function (fCount) {
          if (fCount === 0) {
            addlogs(
              self.config,
              "No folders were found in the stack!",
              "success"
            );
            return resolve();
          }
          ...
          return self
            ...
            .catch(function (error) {
              addlogs(
                self.config,
                chalk.red("Error while exporting asset-folders!"),
                "error"
              );
              return reject(error);
            });
  ...
  getFolderJSON(skip, fCount) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      self.client
        .stack({
          api_key: self.config.source_stack,
          management_token: self.config.management_token,
        })
        .asset()
  ...
  getVersionedAssetJSON(uid, version, bucket) {
    ...
    return new Promise(function (resolve, reject) {
      if (
        self.assetDownloadRetry[uid + version] > self.assetDownloadRetryLimit
      ) {
        console.log("Reached max", self.assetDownloadRetry[uid + version]);
        return reject(
          new Error("Asset Max download retry limit exceeded! " + uid)
        );
      }

      if (version <= 0) {
        const assetVersionInfoFile = path.resolve(
          self.assetsFolderPath,
          uid,
          "_contentstack_" + uid + ".json"
        );
    ...
    return new Promise(async function (resolve, reject) {
      ...
      self.assetStream = {
        url: self.config.securedAssets
          ? `${asset.url}?authtoken=${
              self.config.authtoken || self.config.auth_token
            }`
          : asset.url,
      };
        ...
        .then(function (count) {
          ...
          return self
            ...
            .then(function () {
              addlogs(
                self.config,
                chalk.green("Exported asset-folders successfully!"),
                "success"
              );
              return resolve();
            })
