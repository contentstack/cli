...
describe('auth:tokens:add', () => {
  let promptIndex = 0
  let promptMock = () => {
    let prompts = ['myalias', '***REMOVED***', 'authtokenblt123', 'development']
  ...
  describe('Adding delivery token', () => {
    ...
    test
    .stub(cli, 'prompt', () => async () => '***REMOVED***')
    .command(['auth:tokens:add', '-d', '-a', 'myalias', '-k', '***REMOVED***', '-t', 'authtokenblt123', '-f', '-e', 'development'])
    // eslint-disable-next-line no-unused-vars
  ...
  describe('Adding management token', () => {
    ...
    test
    .stub(cli, 'prompt', () => async () => '***REMOVED***')
    .command(['auth:tokens:add', '-a', 'myalias', '-k', '***REMOVED***', '-t', 'authtokenblt123', '-f'])
    // eslint-disable-next-line no-unused-vars
    .it('should runs auth:tokens:add with command line argument', ctx => {
      let managementTokens = config.get('tokens.myalias')
      assert.equal(managementTokens.token, 'authtokenblt123')
