...
import EntriesImportSetup from '../../../src/import/modules/entries';
import * as loggerModule from '../../../src/utils/logger';
import { ImportConfig } from '../../../src/types';

describe('EntriesImportSetup', () => {
  let entriesSetup: EntriesImportSetup;
  ...
  beforeEach(() => {
    ...
    entriesSetup = new EntriesImportSetup({
    ...
    setupDependenciesStub = stub(entriesSetup, 'setupDependencies').resolves();
  });
  ...
  it('should initialize with the provided config and client', () => {
    expect(entriesSetup.config).to.equal(baseConfig);
    expect(entriesSetup.stackAPIClient).to.equal(mockStackAPIClient);
    expect(entriesSetup.dependencies).to.deep.equal({} as any);
  });

  it('should call setupDependencies during start', async () => {
    await entriesSetup.start();
