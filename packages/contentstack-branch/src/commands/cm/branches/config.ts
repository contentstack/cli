...
import { cliux, messageHandler, printFlagDeprecation, flags, configHandler, logger } from '@contentstack/cli-utilities';
...
export default class BranchConfigCommand extends Command {
  ...
  static usage: string = 'cm:branches:config [-a <value>] [--base-branch <value>] [--stack-api-key <value>] [-y]'; // Note: add all flags
  ...
  async run(): Promise<any> {
    try {
      ...
      if (global) {
        ...
        writeFile(__dirname + '/branch-config.json', branchJson, (err) => {
          if (err) {
            console.error('Failed to write config file: ', err);
          } else console.log('Config file: branch-config.json');
        });
      }
